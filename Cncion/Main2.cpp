...
int main(int argc, char const *argv[])
{
	...
	PaqueteDatagrama p1(sizeof(messageOrder)), p2(sizeof(messageSynchronizeTrack)),p3(sizeof(messageOrder)),p4(sizeof(messageSynchronizeTrack));
	if(order ==0 ){
		sincronizaRequest(order, p1, socketb);
		p2.inicializaIp(ip);	
		p2.inicializaPuerto(7100);
		...
		if(order <3){
			...
			sincronizaRequest(order, p3, socketb);
			p4.inicializaIp(ip);	
			p4.inicializaPuerto(7100);

			}else{
			sincronizaResponse(order,p3, socke);
		}
		//sincronizaResponse(order,p1, socke);
