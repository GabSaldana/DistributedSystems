...
int SocketDatagrama::envia(PaqueteDatagrama &p){
   ...
   std::cout << "Enviando a : " << p.obtieneDireccion() << " por el puerto:"<<p.obtienePuerto()<< std::endl;;
...
int SocketDatagrama::recibe(PaqueteDatagrama &p){
	...
	p.inicializaPuerto(ntohs(direccionForanea.sin_port));
   	...
   	std::cout << "Recibiendo de: " << p.obtieneDireccion() << " por el puerto:"<<p.obtienePuerto()<< std::endl;;
	//std:: cout << " Datos:" <<p.obtieneDatos()<< std::endl;
}
...
int SocketDatagrama::recibeTimeout(PaqueteDatagrama &p){
	...
	if(recv >0){
   	...
   	std::cout << "Recibiendo de: " << p.obtieneDireccion() << " por el puerto:"<<p.obtienePuerto()<< std::endl;;
	//std:: cout << " Datos:" <<p.obtieneDatos()<< std::endl;		
	timersub(&time_ter, &time_ini, &time_res);
	std::cout << "Tiempo de recibo: "<< time_res.tv_sec<<"s "<< time_res.tv_usec<<"us"<< std::endl;
	}
	return ret;
	
}
void SocketDatagrama::tratar_alarma(int opc){
	std::cout << "Fallo Recibo de Paquete"<< std::endl;
}
